...
public class InventoryServiceImpl implements InventoryService {
    ...
    public InventoryServiceImpl(ProductRepository pr,
                                ProductVariantRepository vr,
                                InventoryLogRepository lr) {
        this.productRepo = pr;
        this.variantRepo = vr;
        this.logRepo = lr;
    }

    // === TRANG BIẾN THỂ CỦA 1 SẢN PHẨM ===
    @Override
    ...
    // === TRANG KHO: projection VariantRow ===
    @Override
    public Page<VariantRow> listAllVariantRows(String q, Long productId, Pageable pageable) {
        String kw = null;
        if (StringUtils.hasText(q)) {
            kw = "%" + q.toLowerCase() + "%";
        }
        return variantRepo.searchAllRows(productId, kw, pageable);
    }
}
